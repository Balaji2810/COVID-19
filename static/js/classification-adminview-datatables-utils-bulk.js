$(function () {
    ...
    let table_config = $('#classification_adminview_approval_table_bulk').DataTable({
        lengthChange: false,
        buttons: ['copy', 'excel'],
        "order": [[ 12, "asc" ]]
    });
    ...
    $('#classification_adminview_approval_table_bulk').DataTable();
    $('#classification_adminview_approval_table_bulk tbody').on('click', 'tr', function () {
        $(this).toggleClass('selected active');
    });

    let table = $('#classification_adminview_approval_table_bulk').DataTable();
